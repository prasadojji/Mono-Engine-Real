# backtest_afl.py
"""
Clean standalone backtest runner for AFL strategy.
Loads from DB, aggregates to 5-min, runs AFL strategy, prints only essentials.
"""
import sys
import os
import pandas as pd
...
from mono_engine.strategies.afl_strategy import AFLStrategy  # Correct import from strategies subfolder
# Config — change these as needed
DB_PATH = 'mono_engine_data.db'
DAYS_BACK = 30
TF = '5min'
QUANTITY = 900  # Units per trade for PnL calculation
WATCHLIST_PATH = 'watchlist.json'  # In root folder
def get_watchlist_symbols(watchlist_path: str) -> list:
    if not os.path.exists(watchlist_path):
        print(f"Watchlist file not found: {watchlist_path} — using empty list.")
        return []
    with open(watchlist_path, 'r') as f:
        data = json.load(f)
        # Handle if data is a list of dicts, extract 'symbol' key
        if isinstance(data, list) and all(isinstance(item, dict) for item in data):
            symbols = [item.get('symbol', '') for item in data if 'symbol' in item]
        elif isinstance(data, list) and all(isinstance(item, str) for item in data):
            symbols = data
        else:
            symbols = data.get('symbols', [])  # Fallback to dict
...
def run_backtest_for_symbol(symbol: str, db_path: str, days_back: int, tf: str) -> pd.DataFrame:
    ...
    strategy = AFLStrategy()  # Correct instantiation
    
    trades = []
    min_bars_needed = 30  # Adjust based on your strategy's indicator periods
    ...
    if current_entry is not None:
        ...
        pnl = (last_price - entry_price) * QUANTITY  # Fixed typo (was current_entry['price'])
        cum_pnl += pnl
        trade_summary.append({
            'Symbol': symbol,
            'Quantity': QUANTITY,
    ...
    return summary_df
def run_backtest():
    print(f"\n=== Starting multi-symbol backtest on {TF} ({DAYS_BACK} days) with AFL Strategy ===\n")
    
    symbols = get_watchlist_symbols(WATCHLIST_PATH)
    if not symbols:
        print("No symbols found in watchlist — backtest aborted.")
        return
    
    print(f"Found {len(symbols)} symbols in watchlist: {symbols}")
    ...
    for symbol in symbols:
        print(f"\n--- Backtesting {symbol} with AFL Strategy ---")
