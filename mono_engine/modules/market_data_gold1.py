...
class MarketData(BaseModule):
    ...
    def _sensex_options_workflow(self):
        ...
        with open(options_file, newline='', encoding='utf-8') as f:
            ...
            if self.sensex_expiries:
                logging.info(f"Earliest: {self.sensex_expiries[0]}")
                logging.info(f"Latest:   {self.sensex_expiries[-1]}")
            else:
                logging.warning("No valid Sensex expiries parsed from CSV 'id' field")
            
            # Debug summary
            logging.info(f"Sensex rows matched: {sensex_rows_found}")
            logging.info(f"Expiries successfully parsed: {expiry_parsed_count}")

        # The rest of your method continues unchanged...
    ...
    def load_historical_candles(self):
        ...
        total_symbols = len(historical_items)
