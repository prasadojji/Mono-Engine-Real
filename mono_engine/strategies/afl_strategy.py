...
import numpy as np
import talib
import matplotlib.pyplot as plt  # Keep for now, we'll remove plotting later
...
class AFLStrategy(BaseStrategy):
    """
    Pluggable version of your converted AFL strategy.
    """
    def __init__(self, params: Dict = None):
        ...
        self.base_df = pd.DataFrame()          # Accumulating 1-min bars
        self.internal_strategy = None          # Will hold your Strategy instance
        self.last_entry_idx = -1
        self.last_exit_idx = -1

        print("AFLStrategy initialized - ready for data")

    def on_data_update(self, data: Dict[str, pd.DataFrame]):
        """
        Receives new 1-min candles, accumulates them, and runs your AFL logic.
        """
        if '1min' not in data or data['1min'].empty:
            return

        new_bars = data['1min']
        print(f"Received {len(new_bars)} new 1-min bar(s)")

        # Accumulate
        if self.base_df.empty:
            self.base_df = new_bars.copy()
        else:
            self.base_df = pd.concat([self.base_df, new_bars])
            self.base_df = self.base_df[~self.base_df.index.duplicated(keep='last')]

        print(f"Total accumulated bars: {len(self.base_df)}")

        # Create and run your internal Strategy logic
        try:
            self.internal_strategy = Strategy(self.base_df, params=None)

            # Run the main parts (these will add columns like Buy/Sell)
            # Note: If some methods are missing/truncated, we'll fix in next steps
            #if hasattr(self.internal_strategy, 'run_volume_profile'):
            #    self.internal_strategy.run_volume_profile()
            self.internal_strategy.run_indicators()
            self.internal_strategy.run_trading_logic()

            print("AFL logic ran successfully")
        except Exception as e:
            print(f"Error running strategy logic: {e}")
            import traceback
            traceback.print_exc()  # This will show full error details

    def should_enter(self) -> Tuple[bool, float | None]:
        if self.internal_strategy is None or 'Buy' not in self.internal_strategy.df.columns:
            return False, None

        df = self.internal_strategy.df
        current_idx = len(df) - 1

        buy_now = bool(df['Buy'].iloc[current_idx])
        buy_prev = current_idx > 0 and bool(df['Buy'].iloc[current_idx - 1])
    ...
    def should_exit(self) -> Tuple[bool, float | None]:
        if self.internal_strategy is None or 'Sell' not in self.internal_strategy.df.columns:
            return False, None

        df = self.internal_strategy.df
        current_idx = len(df) - 1

        sell_now = bool(df['Sell'].iloc[current_idx])

        if sell_now and current_idx != self.last_exit_idx:
            self.last_exit_idx = current_idx
            price = df['Close'].iloc[current_idx]
            print(f"EXIT SIGNAL! Suggested price: {price}")
            return True, price

        return False, None

    def reset_day(self):
        print("New trading day - resetting strategy data")
...
class Strategy:
    ...
    def _expand(self, series, base_df, mode='point'):
        ...
        """
        if mode == 'point':
            return series.reindex(base_df.index, method='ffill').ffill()
        elif mode == 'last':
    ...
    def calculate_volume_value_area(self, fb, lb):
        ...
        for i in range(3 + self.params['shft']):
            ...
            lbx[i] = pd.Series(lbx[i]).ffill().values  # Approximate ValueWhen
    ...
    def shift_volume_value_area(self, fb, lb, shft):
        ...
        for i in range(3 + shft):
            ...
            for shift in range(1, i + 1):
                lbx[i] = self._ref(lbx[i], -1)
            lbx[i] = pd.Series(lbx[i]).ffill().values
    ...
    def run_volume_profile(self):
        ...
        for i in range(3 + self.params['shft']):
            ...
            self._var_set(f"lbx{i}", pd.Series(self._var_get(f"lbx{i}")).ffill().values)
    ...
    def run_indicators(self):
        ...
        # Volume POC/PONC (simplified rolling) - already fixed in previous step
        ...
        df_5min['highestVolPrice'] = np.where(df_5min['Volume'] == highest_vol, df_5min['Close'], np.nan)
        df_5min['highestVolPrice'] = df_5min['highestVolPrice'].ffill()
        
        df_5min['lowestVolPrice'] = np.where(df_5min['Volume'] == lowest_vol, df_5min['Close'], np.nan)
        df_5min['lowestVolPrice'] = df_5min['lowestVolPrice'].ffill()
        ...
        # FIXED: Convert to float for talib
        vol_imbalance = talib.EMA((green_vol - red_vol).astype(float), 5) / talib.EMA(df_5min['Volume'].astype(float), 5)
        ...
        # All conditions (no skips - full translation) - FIXED for NaN & type safety
        df_5min['STD'] = (df_5min['vwapValue'] > self._ref(df_5min['vwapValue'], -1)).fillna(False) & df_5min['VolumeSpike'].fillna(False)
        df_5min['GC'] = (self._ref(df_5min['Open'], -1) < self._ref(df_5min['Close'], -1)).fillna(False)
        df_5min['RC'] = (self._ref(df_5min['Open'], -1) > self._ref(df_5min['Close'], -1)).fillna(False)
        df_5min['B0'] = (df_5min['High'] > self._ref(df_5min['High'], -1)).fillna(False)
        df_5min['B1'] = (self._ref(df_5min['StrengthPct'], -1) > 65).fillna(False) & df_5min['GC']
        df_5min['B101'] = (self._ref(df_5min['StrengthPct'], -1) < 10).fillna(False) & ((df_5min['RC'] | df_5min['GC']).fillna(False))
        df_5min['B2'] = (self._ref(df_5min['UpperWick'], -1) > self._ref(df_5min['LowerWick'], -1)).fillna(False)
        df_5min['B21'] = (self._ref(df_5min['UpperWick'], -1) < self._ref(df_5min['LowerWick'], -1)).fillna(False)
        df_5min['B3'] = (self._ref(df_5min['CandleStrength'], -1) > 0).fillna(False)
        df_5min['B31'] = (self._ref(df_5min['CandleStrength'], -1) > 120000).fillna(False)
        df_5min['B32'] = (self._ref(df_5min['CandleStrength'], -1) > 500000).fillna(False)
        df_5min['B4'] = (self._ref(df_5min['ATR_Strength'], -1) > 100).fillna(False)
        df_5min['B41'] = (self._ref(df_5min['ATR_Strength'], -1) > 80).fillna(False)
        df_5min['B5'] = (self._ref(df_5min['RSIValue'], -1) > 39).fillna(False)
        df_5min['B6'] = self._ref(df_5min['UpTrend'], -1).fillna(False)
        df_5min['B7'] = self._ref(df_5min['BullishTrend'], -1).fillna(False)
        df_5min['B8'] = self._ref(df_5min['BullishTrendVol'], -1).fillna(False)
        df_5min['B9'] = df_5min['ConfirmedBuy'].fillna(False)
        df_5min['B10'] = self._ref(df_5min['DonTrendFilter'], -1).fillna(False)
        df_5min['B11'] = df_5min['B101'] & df_5min['B21']
        df_5min['B12'] = (self._ref(df_5min['ATR3P'], -1) > 50).fillna(False)
        ...
        # Sell conditions (full, no skips) - FIXED
        df_5min['S1'] = (self._ref(df_5min['High'], -1) == self._ref(df_5min['Close'], -1)).fillna(False) & \
                        (df_5min['Open'] > self._ref(df_5min['Close'], -1)).fillna(False) & \
                        (self._ref(df_5min['Low'], -1) < df_5min['Close']).fillna(False) & df_5min['GC']
        df_5min['S2'] = (self._ref(df_5min['High'], -1) == self._ref(df_5min['Close'], -1)).fillna(False) & \
                        (df_5min['Open'] > self._ref(df_5min['High'], -1)).fillna(False) & \
                        (self._ref(df_5min['Low'], -1) < df_5min['Close']).fillna(False) & \
                        (df_5min['Low'] < (self._ref(df_5min['Low'], -1) - self._ref(df_5min['Low'], -1) * 0.02)).fillna(False) & \
                        (df_5min['Open'] - df_5min['Low'] > 2).fillna(False) & df_5min['GC']
        df_5min['S3'] = (df_5min['MACDLine'] < df_5min['SignalLine']).fillna(False) & \
                        (df_5min['RSIValue'] < 50).fillna(False) & \
                        (df_5min['adxValue'] < 25).fillna(False) & \
                        (df_5min['SlowK'] >= 80).fillna(False) & df_5min['GC']
        df_5min['S4'] = (df_5min['Open'] > self._ref(df_5min['High'], -1)).fillna(False) & \
                        (self._ref(df_5min['Open'], -1) != self._ref(df_5min['Low'], -1)).fillna(False) & \
                        (df_5min['Low'] < self._ref(df_5min['Close'], -1) - 6).fillna(False) & \
                        (df_5min['Low'] < (self._ref(df_5min['Low'], -1) - self._ref(df_5min['Low'], -1) * 0.02)).fillna(False) & \
                        (df_5min['Open'] - df_5min['Low'] > 2).fillna(False) & df_5min['GC']
        df_5min['S5'] = (self._ref(df_5min['Open'], -1) - self._ref(df_5min['Low'], -1) < 1).fillna(False) & \
                        (df_5min['Low'] < self._ref(df_5min['Low'], -1) - (self._ref(df_5min['Open'], -1) + self._ref(df_5min['Close'], -1)) / 2 * 0.03).fillna(False) & \
                        (df_5min['Low'] < (self._ref(df_5min['Low'], -1) - self._ref(df_5min['Low'], -1) * 0.02)).fillna(False) & \
                        (df_5min['Open'] - df_5min['Low'] > 2).fillna(False) & df_5min['GC']
        df_5min['S6'] = (self._ref(df_5min['High'], -1) < self._ref(df_5min['High'], -2)).fillna(False) & df_5min['GC']
        df_5min['S7'] = (self._ref(df_5min['Low'], -1) < self._ref(df_5min['Low'], -2)).fillna(False) & \
                        (df_5min['Low'] < self._ref(df_5min['Low'], -1) - 2).fillna(False) & \
                        (df_5min['Low'] < (self._ref(df_5min['Low'], -1) - self._ref(df_5min['Low'], -1) * 0.02)).fillna(False) & \
                        (df_5min['Open'] - df_5min['Low'] > 2).fillna(False) & df_5min['GC']
        df_5min['S8'] = (df_5min['Low'] < self._ref(df_5min['Open'], -1)).fillna(False) & \
                        (df_5min['Low'] < self._ref(df_5min['Open'], -2)).fillna(False) & \
                        (df_5min['Low'] < self._ref(df_5min['Low'], -1)).fillna(False) & \
                        (df_5min['Low'] < self._ref(df_5min['Low'], -2)).fillna(False) & \
                        (self._ref(df_5min['Open'], -2) > self._ref(df_5min['Close'], -2)).fillna(False) & \
                        (df_5min['Low'] < (self._ref(df_5min['Low'], -1) - self._ref(df_5min['Low'], -1) * 0.02)).fillna(False) & \
                        (df_5min['Open'] - df_5min['Low'] > 2).fillna(False) & df_5min['GC']
        df_5min['S9'] = (df_5min['Low'] < self._ref(df_5min['Low'], -1) - 2).fillna(False) & \
                        (df_5min['Low'] < self._ref(df_5min['Open'], -2) - 3).fillna(False) & \
                        (df_5min['Low'] < self._ref(df_5min['Low'], -1) - self._ref(df_5min['Low'], -1) * 0.04).fillna(False) & \
                        (df_5min['Open'] - df_5min['Low'] > 2).fillna(False) & df_5min['GC']
        df_5min['S3R'] = (df_5min['MACDLine'] < df_5min['SignalLine']).fillna(False) & \
                         (df_5min['RSIValue'] < 50).fillna(False) & \
                         (df_5min['adxValue'] < 25).fillna(False) & \
                         (df_5min['SlowK'] >= 80).fillna(False) & df_5min['RC']
        df_5min['SR1'] = (df_5min['Open'] < self._ref(df_5min['Low'], -1)).fillna(False) & \
                         (df_5min['Open'] - df_5min['Low'] > 2).fillna(False) & df_5min['RC']
        df_5min['SR2'] = (self._ref(df_5min['Close'], -1) - self._ref(df_5min['Low'], -1) < 0.4).fillna(False) & \
                         (df_5min['Low'] < self._ref(df_5min['Low'], -1)).fillna(False) & \
                         (self._ref(df_5min['Low'], -1) - self._ref(df_5min['Low'], -1) * 0.02).fillna(False) & \
                         (df_5min['Open'] - df_5min['Low'] > 2).fillna(False) & df_5min['RC']
        df_5min['SR3'] = df_5min['RC'] & \
                         (self._ref(df_5min['Open'], -1) - self._ref(df_5min['Close'], -1) < self._ref(df_5min['High'], -1) - self._ref(df_5min['Open'], -1)).fillna(False) & \
                         (self._ref(df_5min['High'], -1) - self._ref(df_5min['Open'], -1) > self._ref(df_5min['Close'], -1) - self._ref(df_5min['Low'], -1)).fillna(False) & \
                         (self._ref(df_5min['Close'], -1) - self._ref(df_5min['Low'], -1) < self._ref(df_5min['Open'], -1) - self._ref(df_5min['Close'], -1)).fillna(False) & \
                         (df_5min['Low'] < self._ref(df_5min['Low'], -1)).fillna(False) & \
                         (df_5min['Open'] - df_5min['Low'] > 2).fillna(False)
        df_5min['SR4'] = (df_5min['High'] - df_5min['Open'] < 2).fillna(False) & \
                         (df_5min['Low'] < self._ref(df_5min['Low'], -1)).fillna(False) & df_5min['RC']
        df_5min['SR5'] = (self._ref(df_5min['High'], -1) > self._ref(df_5min['High'], -2)).fillna(False) & \
                         (self._ref(df_5min['Low'], -1) > self._ref(df_5min['Low'], -2)).fillna(False) & \
                         (df_5min['Open'] > self._ref(df_5min['Close'], -1)).fillna(False) & \
                         (df_5min['Open'] - self._ref(df_5min['Close'], -1) > 0.5).fillna(False) & df_5min['RC']  # Removed BuyPrice 6% check (handled by engine)

        df_5min['SR6'] = df_5min['SR5']  # Still duplicate as in original

        df_5min['SR7'] = (df_5min['Open'] < self._ref(df_5min['Close'], -1)).fillna(False) & \
                         (df_5min['Low'] < self._ref(df_5min['Low'], -1) - 3).fillna(False) & \
                         (df_5min['Low'] < self._ref(df_5min['Low'], -1) - self._ref(df_5min['Low'], -1) * 0.02).fillna(False) & \
                         (df_5min['Open'] - df_5min['Low'] > 2).fillna(False) & df_5min['RC']
        df_5min['SR8'] = (df_5min['Low'] < self._ref(df_5min['Low'], -1) - 3).fillna(False) & \
                         (self._ref(df_5min['Open'], -2) > self._ref(df_5min['Close'], -2)).fillna(False) & \
                         (df_5min['Low'] <= self._ref(df_5min['Low'], -2)).fillna(False) & \
                         (df_5min['Low'] < self._ref(df_5min['Low'], -1) - self._ref(df_5min['Low'], -1) * 0.02).fillna(False) & \
                         (df_5min['Open'] - df_5min['Low'] > 2).fillna(False) & df_5min['RC']
        df_5min['SR9'] = (self._ref(df_5min['High'], -1) > self._ref(df_5min['High'], -2)).fillna(False) & \
                         (self._ref(df_5min['Low'], -1) > self._ref(df_5min['Low'], -2)).fillna(False) & df_5min['RC']  # Removed BuyPrice 6% check (handled by engine)
       
        # Combo formulas (full, no skips) - FIXED
        df_5min['CB1'] = df_5min['GC'] & (df_5min['MACDLine'] > df_5min['SignalLine']).fillna(False) & (df_5min['RSIValue'] > 40).fillna(False) & (df_5min['adxValue'] > 10).fillna(False) & (df_5min['SlowK'] > df_5min['SlowD']).fillna(False) & (df_5min['High'] >= df_5min['MiddleBand']).fillna(False)
        df_5min['CB2'] = df_5min['GC'] & (df_5min['MACDLine'] > 5).fillna(False) & (df_5min['RSIValue'] > 40).fillna(False) & (df_5min['adxValue'] > 10).fillna(False) & (df_5min['SlowK'] > df_5min['SlowD']).fillna(False) & (df_5min['High'] >= df_5min['MiddleBand']).fillna(False)
        df_5min['CB3'] = df_5min['GC'] & (df_5min['Open'] < df_5min['LowerBand']).fillna(False) & (df_5min['High'] >= df_5min['MiddleBand']).fillna(False)
        df_5min['CB4'] = (df_5min['RSIValue'] > 20).fillna(False) & (df_5min['SlowK'] > df_5min['SlowD']).fillna(False) & (df_5min['adxValue'] > 40).fillna(False) & (df_5min['High'] >= df_5min['MiddleBand']).fillna(False)
        df_5min['CB5'] = df_5min['GC'] & (df_5min['MACDLine'] > df_5min['SignalLine']).fillna(False) & (df_5min['RSIValue'] > 40).fillna(False) & (df_5min['adxValue'] > 20).fillna(False) & (df_5min['Open'] >= df_5min['LowerBand']).fillna(False)
        df_5min['CB6'] = df_5min['GC'] & (df_5min['RSIValue'] > 40).fillna(False) & (df_5min['adxValue'] > 19).fillna(False) & (df_5min['Open'] >= df_5min['LowerBand']).fillna(False) & (df_5min['SlowK'] > df_5min['SlowD']).fillna(False)
        df_5min['CBR1'] = df_5min['RC'] & (df_5min['MACDLine'] > df_5min['SignalLine']).fillna(False) & (df_5min['RSIValue'] > 40).fillna(False) & (df_5min['adxValue'] > 10).fillna(False) & (df_5min['SlowK'] > df_5min['SlowD']).fillna(False) & (df_5min['High'] >= df_5min['MiddleBand']).fillna(False) & (df_5min['Open'] < df_5min['UpperBand']).fillna(False) & (df_5min['SlowK'] - df_5min['SlowD'] < 1.6).fillna(False)
        df_5min['CBR2'] = df_5min['RC'] & (df_5min['MACDLine'] > 5).fillna(False) & (df_5min['RSIValue'] > 40).fillna(False) & (df_5min['adxValue'] > 10).fillna(False) & (df_5min['Open'] < df_5min['UpperBand']).fillna(False) & (df_5min['SlowK'] - df_5min['SlowD'] < 1.6).fillna(False) & (df_5min['High'] >= df_5min['MiddleBand']).fillna(False)
        df_5min['CBR3'] = df_5min['RC'] & (df_5min['Open'] < self._ref(df_5min['LowerBand'], -1)).fillna(False) & (df_5min['Open'] > df_5min['UpperBand']).fillna(False) & (df_5min['SlowK'] - df_5min['SlowD'] < 1.6).fillna(False) & (df_5min['High'] >= df_5min['MiddleBand']).fillna(False)
        df_5min['CBR4'] = df_5min['RC'] & (df_5min['MACDLine'] > df_5min['SignalLine']).fillna(False) & (df_5min['RSIValue'] > 40).fillna(False) & (df_5min['adxValue'] > 20).fillna(False) & (df_5min['Open'] >= df_5min['LowerBand']).fillna(False)
        df_5min['BR1'] = df_5min['STD'] & df_5min['RC'] & (df_5min['MACDLine'] > df_5min['SignalLine']).fillna(False) & (df_5min['RSIValue'] > 40).fillna(False) & (df_5min['adxValue'] > 25).fillna(False) & (df_5min['SlowK'] > df_5min['SlowD']).fillna(False) & (df_5min['Open'] < df_5min['UpperBand']).fillna(False) & (df_5min['High'] >= df_5min['MiddleBand']).fillna(False) & (df_5min['SlowK'] - df_5min['SlowD'] > 1.6).fillna(False)
        df_5min['BR2'] = df_5min['STD'] & df_5min['RC'] & (df_5min['MACDLine'] > 5).fillna(False) & (df_5min['RSIValue'] > 40).fillna(False) & (df_5min['adxValue'] > 25).fillna(False) & (df_5min['Open'] < df_5min['UpperBand']).fillna(False) & (df_5min['SlowK'] - df_5min['SlowD'] > 1.6).fillna(False) & (df_5min['High'] >= df_5min['MiddleBand']).fillna(False)
        df_5min['BR3'] = df_5min['STD'] & df_5min['RC'] & (df_5min['Open'] < df_5min['LowerBand']).fillna(False) & (df_5min['Open'] < df_5min['UpperBand']).fillna(False) & (df_5min['SlowK'] - df_5min['SlowD'] > 1.6).fillna(False) & (df_5min['High'] >= df_5min['MiddleBand']).fillna(False)
        df_5min['BR4'] = df_5min['STD'] & df_5min['RC'] & (df_5min['MACDLine'] > df_5min['SignalLine']).fillna(False) & (df_5min['RSIValue'] > 40).fillna(False) & (df_5min['adxValue'] > 20).fillna(False) & (df_5min['Open'] >= df_5min['LowerBand']).fillna(False) & (df_5min['High'] >= df_5min['MiddleBand']).fillna(False)
        df_5min['BR5'] = df_5min['STD'] & df_5min['RC'] & (df_5min['Low'] < df_5min['LowerBand']).fillna(False) & (df_5min['High'] > self._ref(df_5min['High'], -1)).fillna(False)
        df_5min['BR6'] = (self._ref(df_5min['Low'], -1) < self._ref(df_5min['LowerBand'], -1)).fillna(False) & (self._ref(df_5min['Close'], -1) > self._ref(df_5min['LowerBand'], -1)).fillna(False)
        df_5min['NRC1'] = (df_5min['High'] > self._ref(df_5min['High'], -1) + 3).fillna(False) & (df_5min['High'] > self._ref(df_5min['High'], -2)).fillna(False)
        df_5min['NRC2'] = (self._ref(df_5min['High'], -1) > self._ref(df_5min['High'], -2)).fillna(False) & (self._ref(df_5min['Low'], -1) > self._ref(df_5min['Low'], -2)).fillna(False) & (df_5min['Open'] > self._ref(df_5min['Close'], -1)).fillna(False) & (self._ref(df_5min['Close'], -2) != self._ref(df_5min['High'], -2)).fillna(False) & (df_5min['High'] > (self._ref(df_5min['Open'], -1) + self._ref(df_5min['Close'], -1)) / 2).fillna(False)
        df_5min['NRC3'] = (self._ref(df_5min['High'], -1) - self._ref(df_5min['Open'], -1) < self._ref(df_5min['Close'], -1) - self._ref(df_5min['Low'], -1)).fillna(False) & (self._ref(df_5min['Open'], -1) - self._ref(df_5min['Close'], -1) <= 2).fillna(False) & (df_5min['High'] > self._ref(df_5min['High'], -1) + 3).fillna(False)
        df_5min['NRC4'] = (df_5min['Open'] > self._ref(df_5min['Close'], -1)).fillna(False) & (df_5min['High'] > self._ref(df_5min['Open'], -1)).fillna(False) & (self._ref(df_5min['Open'], -2) > self._ref(df_5min['Close'], -2)).fillna(False) & (df_5min['High'] > self._ref(df_5min['Open'], -2)).fillna(False) & (df_5min['High'] > self._ref(df_5min['High'], -1) + 3).fillna(False)
        df_5min['NRC5'] = (self._ref(df_5min['High'], -1) == self._ref(df_5min['Open'], -1)).fillna(False) & (df_5min['Open'] > self._ref(df_5min['Close'], -1)).fillna(False)
        df_5min['NRCB'] = ((df_5min['BR1'] | df_5min['BR2'] | df_5min['BR3'] | df_5min['BR4'] | df_5min['BR5'] | df_5min['BR6']).fillna(False)) & ((df_5min['NRC1'] | df_5min['NRC2'] | df_5min['NRC3'] | df_5min['NRC4']).fillna(False))
        df_5min['BB1'] = df_5min['B1'] & df_5min['B2'] & df_5min['B3'] & df_5min['B5']
        df_5min['BB2'] = ((df_5min['B2'] & df_5min['B3'] & df_5min['B4'] & df_5min['B5'] & df_5min['B6']).fillna(False)) | ((df_5min['B2'] & df_5min['B3'] & df_5min['B4']).fillna(False))
        ...
        df_5min['BB7'] = df_5min['B2'] & df_5min['B4'] & df_5min['B5'] & df_5min['B6'] & (~df_5min['RC']).fillna(False)
        ...
        df_5min['BB12'] = (df_5min['SlowK'] > df_5min['SlowD']).fillna(False) & (df_5min['SlowK'] - df_5min['SlowD'] > 3).fillna(False)
        df_5min['BB13'] = df_5min['B11'] & (df_5min['High'] > self._ref(df_5min['High'], -1)).fillna(False)
        df_5min['BB14'] = df_5min['B1'] & df_5min['B2'] & df_5min['B32'] & df_5min['B0'] & df_5min['B12']
        df_5min['BB15'] = df_5min['B2'] & df_5min['B3'] & df_5min['B12'] & df_5min['GC'] & (df_5min['High'] > self._ref(df_5min['High'], -1)).fillna(False)
        df_5min['BB16'] = df_5min['B21'] & df_5min['B41'] & df_5min['B12'] & df_5min['RC'] & (df_5min['High'] > self._ref(df_5min['High'], -1)).fillna(False)
        df_5min['BB17'] = df_5min['GC'] & (df_5min['High'] > self._ref(df_5min['High'], -1)).fillna(False) & (self._ref(df_5min['Open'], -1) - self._ref(df_5min['Low'], -1) < 1).fillna(False) & (df_5min['CSS'] > 70).fillna(False)
        df_5min['CBB0'] = df_5min['CB1'] | df_5min['CB2'] | df_5min['CB3'] | df_5min['CB4'] | df_5min['CB5'] | df_5min['CB6']
        df_5min['CBB1'] = df_5min['CBB0'] & (df_5min['High'] > self._ref(df_5min['High'], -1) + 3).fillna(False) & (df_5min['Open'] > self._ref(df_5min['Open'], -1)).fillna(False) & (self._ref(df_5min['High'], -1) > self._ref(df_5min['High'], -2)).fillna(False) & (self._ref(df_5min['Low'], -1) > self._ref(df_5min['Low'], -2)).fillna(False)
        df_5min['CBB2'] = df_5min['CBB0'] & (df_5min['Open'] < self._ref(df_5min['Close'], -1)).fillna(False) & (self._ref(df_5min['Low'], -1) > self._ref(df_5min['Low'], -2)).fillna(False) & (df_5min['High'] > self._ref(df_5min['High'], -1) + 3).fillna(False) & (df_5min['High'] > self._ref(df_5min['High'], -2)).fillna(False)
        df_5min['CBB3'] = df_5min['CBB0'] & (df_5min['Open'] > self._ref(df_5min['Close'], -1)).fillna(False) & (df_5min['High'] > self._ref(df_5min['High'], -1) + 3).fillna(False) & (self._ref(df_5min['Open'], -1) > self._ref(df_5min['Close'], -2)).fillna(False)
        df_5min['CBB4'] = df_5min['CBB0'] & (df_5min['Open'] > self._ref(df_5min['Close'], -1)).fillna(False) & (df_5min['High'] > self._ref(df_5min['High'], -1) + 3).fillna(False) & (self._ref(df_5min['Open'], -1) - self._ref(df_5min['Close'], -2) < 1).fillna(False)
        df_5min['CBB5'] = df_5min['CBB0'] & (df_5min['Open'] < self._ref(df_5min['Close'], -1)).fillna(False) & (self._ref(df_5min['Close'], -1) == self._ref(df_5min['High'], -1)).fillna(False) & (df_5min['High'] > self._ref(df_5min['High'], -1) + 3).fillna(False)
        df_5min['CBB6'] = df_5min['CBB0'] & (df_5min['Open'] < self._ref(df_5min['Close'], -1)).fillna(False) & (df_5min['Open'] - df_5min['Low'] <= (df_5min['Open'] * 0.0100)).fillna(False) & (df_5min['High'] > self._ref(df_5min['High'], -1) + 3).fillna(False)
        df_5min['CBB7'] = df_5min['CBB0'] & (self._ref(df_5min['Open'], -1) == self._ref(df_5min['Low'], -1)).fillna(False) & (df_5min['Open'] < self._ref(df_5min['Close'], -1)).fillna(False) & (df_5min['High'] > self._ref(df_5min['High'], -1) + 3).fillna(False) | \
                          df_5min['CBB0'] & (self._ref(df_5min['High'], -1) > self._ref(df_5min['High'], -2)).fillna(False) & (df_5min['Open'] < self._ref(df_5min['Close'], -1)).fillna(False) & (df_5min['High'] > self._ref(df_5min['High'], -1) + 3).fillna(False)
        df_5min['CBB8'] = df_5min['CBB0'] & (self._ref(df_5min['Close'], -1) - self._ref(df_5min['Open'], -1) > 9).fillna(False) & (self._ref(df_5min['Close'], -1) - self._ref(df_5min['Open'], -1) < 13).fillna(False)
        df_5min['CBR'] = df_5min['CBR1'] | df_5min['CBR2'] | df_5min['CBR3'] | df_5min['CBR4']
        df_5min['CBBR1'] = df_5min['CBR'] & (df_5min['Open'] > self._ref(df_5min['High'], -1)).fillna(False)
        df_5min['CBBR2'] = df_5min['CBR'] & (self._ref(df_5min['High'], -1) - self._ref(df_5min['Open'], -1) < 0.5).fillna(False)
        df_5min['CBBR3'] = df_5min['CBR'] & (df_5min['High'] > self._ref(df_5min['High'], -1) + 3).fillna(False) & (df_5min['High'] > self._ref(df_5min['High'], -2)).fillna(False) & (df_5min['High'] > self._ref(df_5min['High'], -1) + 3).fillna(False)
        df_5min['CBBR4'] = df_5min['CBR'] & (self._ref(df_5min['High'], -1) > self._ref(df_5min['High'], -2)).fillna(False) & (self._ref(df_5min['Low'], -1) > self._ref(df_5min['Low'], -2)).fillna(False) & (df_5min['Open'] > self._ref(df_5min['Close'], -1)).fillna(False) & (self._ref(df_5min['Close'], -2) != self._ref(df_5min['High'], -2)).fillna(False) & (df_5min['High'] > (self._ref(df_5min['Open'], -1) + self._ref(df_5min['Close'], -1)) / 2).fillna(False) & (df_5min['High'] > self._ref(df_5min['High'], -1)).fillna(False)
        df_5min['CBBR5'] = df_5min['CBR'] & (self._ref(df_5min['High'], -1) - self._ref(df_5min['Open'], -1) < self._ref(df_5min['Close'], -1) - self._ref(df_5min['Low'], -1)).fillna(False) & (self._ref(df_5min['Open'], -1) - self._ref(df_5min['Close'], -1) <= 2).fillna(False) & (df_5min['High'] > self._ref(df_5min['High'], -1) + 3).fillna(False)
        df_5min['CBBR6'] = df_5min['CBR'] & (df_5min['Open'] > self._ref(df_5min['Close'], -1)).fillna(False) & (df_5min['High'] > self._ref(df_5min['Open'], -1)).fillna(False) & (self._ref(df_5min['Open'], -2) > self._ref(df_5min['Close'], -2)).fillna(False) & (df_5min['High'] > self._ref(df_5min['Open'], -2)).fillna(False) & (df_5min['High'] > self._ref(df_5min['High'], -1) + 3).fillna(False)
        df_5min['CBBR7'] = df_5min['CBR'] & (self._ref(df_5min['High'], -1) == self._ref(df_5min['Open'], -1)).fillna(False) & (df_5min['Open'] > self._ref(df_5min['Close'], -1)).fillna(False)
        ...
        df_5min['SSS'] = (df_5min['MACDLine'] < df_5min['SignalLine']).fillna(False) & (df_5min['RSIValue'] < 50).fillna(False) & (df_5min['adxValue'] < 25).fillna(False) & (df_5min['SlowK'] >= 80).fillna(False)
        df_5min['SSN'] = df_5min['SSR1'] | df_5min['SSR2'] | df_5min['SSR3'] | df_5min['SSR4'] | df_5min['SSR5'] | df_5min['SSR6'] | (~df_5min['SSR7']).fillna(False)
    ...
    def run_trading_logic(self):
        ...
        """
        # Green, Red, MA1 (full, no skips) - already calculated in indicators, but repeat for safety
        ...
        # Vectorized Buy/Sell (rising/falling edge of MA1)
        self.df['Buy'] = (self.df['MA1']) & (~self._ref(self.df['MA1'], -1).fillna(False))
        self.df['Sell'] = (~self.df['MA1']) & (self._ref(self.df['MA1'], -1).fillna(False))
        
        # Optional: Add simple exit on Red if in Green, etc. (expand later)
        self.df['Sell'] = self.df['Sell'] | self.df['Red']
        
        # ExRem equivalent (remove extra buys/sells when alternating)
        buy = self.df['Buy'].copy()
        sell = self.df['Sell'].copy()
        buy[buy.cumsum() > sell.cumsum().shift(fill_value=0)] = False
        sell[sell.cumsum() > buy.cumsum().shift(fill_value=0)] = False
        self.df['Buy'] = buy
        self.df['Sell'] = sell
        
        print("Trading logic completed - Buy/Sell columns generated")
...
if __name__ == "__main__":
    ...
    # Create fake 1-min data (200 bars, random prices around 100)
    start_time = datetime.now().replace(second=0, microsecond=0) - timedelta(minutes=199)
    dates = pd.date_range(start_time, periods=200, freq='1min')
    
    np.random.seed(42)  # For reproducible data
    opens = np.random.uniform(95, 105, 200)
    closes = opens + np.random.uniform(-2, 2, 200)
    highs = np.maximum(opens, closes) + np.random.uniform(0, 2, 200)
    lows = np.minimum(opens, closes) - np.random.uniform(0, 2, 200)
    ...
    # Split into small batches to simulate live data coming in
    strategy = AFLStrategy()

    for i in range(0, len(fake_df), 10):  # Send 10 bars at a time
        batch = fake_df.iloc[i:i+10]
        print("\n--- Feeding batch ---")
        ...
        if enter:
            print(f"*** BUY SIGNAL at {batch.index[-1]} - Price: {price} ***")
        
        exit_, price = strategy.should_exit()
        if exit_:
            print(f"*** SELL SIGNAL at {batch.index[-1]} - Price: {price} ***")
